...
ONE = {}
ONE.关键词 = {}
ONE.关键词[1] = {'芜湖麻将',3,50000,50}
------------------应用包信息--------------------------------
ONE.应用名={'芜湖麻将','com.wuhumj.ios'}
----------------------------------------way = 2 ------------------------------------------
w,h=getScreenSize()
if w > 640 then
	require "ASO-iphone6"
...
m.有结果={ 0x0080fc, "-36|-26|0x0080fc,-38|23|0x0080fc", 90, 489, 148, 622, 1136}
...
m['17岁'] = { 0x000000, "0|17|0x000000,6|0|0x000000,17|0|0x202221", 90, 347, 484, 373, 603}
m['精品推荐']={{566,71,0x007aff},{566,75,0xf9f9f9},{566,82,0x007aff},{566,93,0x007aff},{574,94,0x007aff},}
...
function login()
	...
	while (true) do
		...
		elseif login_setp == 4 then
			...
			login_ok,loginlast = 0,0
		elseif login_setp == 5 then
			loginlast = loginlast + 1
	...
	while (true) do
		...
		elseif sotp == search_setp then
			if search_setp == 3 then
				wait_mun = 500 + move_times * 60 --由于排名没有200名以外的 所以滑动时间应该小于300秒
			else
				wait_mun = 180
			end
...
if way == 1 then
	...
	for i,v in ipairs(ONE.关键词) do
		...
		while mun < end_mun do
			if login()then
				move_down_time = ONE.关键词[i][2]/5-20  --批量设置滑动时间
