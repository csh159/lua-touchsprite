...
--[[
vpnKey = '0'  --vpn等0 则是启用vpn 其它就是不启动vpn
starKey = '0' --星级选择0为5星，
--]]
...
ocrInfo("haoi23","zhijiankeji","zhijiankeji|ED4EF20372B5A3C6")
...
m.简短的验证出现验证码 ={ 0x000000, 90, 91, 470, 555, 573}
	...
	while os.time()-outTime < 60 * 3 do
		...
		elseif setp == 3 then
			if f_pic(m.评论激活,"评论激活") and f_pic(m.撰写评论,'撰写评论')then
				nLog('评论发送成功')
				return true
			elseif CommentStar()then  ---判断5星 为灰色则点击
...
function other()
	if done(m.灰,'有弹窗-进入错误处理')then
		...
		elseif done(m['需要验证'],'需要验证')then				--ok
			click(446,665)
		elseif c_pic(m.简短的验证,"简短的验证")then
...
------------------------------------帐号处理问题------------------------
		...
		elseif  done(m.简短的验证界面,'简短的验证界面')then
			if f_p(m.简短的验证出现验证码,"简短的验证出现验证码")then
