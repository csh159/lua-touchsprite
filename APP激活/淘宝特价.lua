...
function upidfa(api_Bid)
	...
	if abl()then
		...
		if VPN()then
			...
			if ip_address then
				if GetTask(api_Bid,adid,idfa)then
					toast('排重成功',1)
					up ={}
					up.mail = idfa
					up.fristname = app_name
...
bid=tbtjb&idfa=DD4AD0C4-91F2-4106-BF7C-5AC563DDC1C9&source=sz&adid=10069&ip=192.168.5.6
--]]

if xz(bid_app)then
	while (true) do
		if upidfa('tbtjb')then
