	...
	if abl()then
		if get_idfa(bid_app)then
			...
			if VPN()then
				...
				toast(ip_address,1)
				if ip_address then
					if GetTask("cctv",adid,idfa)then
						toast('排重成功',1)
						up ={}
						up.mail = idfa
...
app_name = 'CCTV微视——央视春晚体育手机直播'
bid_app = 'com.CC.WeSee'
adid = '10106'
id = 16649
...
app_url = 'https://itunes.apple.com/cn/app/cctv%E5%BE%AE%E8%A7%86/id750958218?mt=8'
