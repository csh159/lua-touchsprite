...
aoc['lls']['选择城市']={{307,67,0x191919},{56,79,0xe6454a},{180,251,0x424242},}
...
aoc['lls']['请输入验证码']={{422,67,0x333333},{36,84,0xe6454a},{416,71,0xdcdcdc},{416,72,0x333333},}
	aoc['lls']['有新人礼']={{12,134,0xef7c4d},{625,138,0xef7c4e},}	
	aoc['lls']['登录按钮']={ 0xfef7f8, "-10|-2|0xe6454a,-13|-26|0xe6454a", 90, 176, 636, 462, 737}
	aoc['lls']['登录按钮2']={ 0xfef7f8, "-10|-2|0xe6454a,-13|-26|0xe6454a", 90, 237, 494, 403, 560}
...
function reg()
	...
	while (os.time()-计时<超时) do
		...
		elseif done(aoc['lls']['请输入验证码'],'请输入验证码',false,1)then
			...
			if 获取 then
				...
				while (true) do
					if getMessage(id,phone,token)then
						if done(aoc['lls']['有新人礼'],'有新人礼',false,1)then
							click(369,410)
							input(dxyzm)
							click(443,307)
							获取 = false
			...
			if done(aoc['lls']['有新人礼'],'有新人礼',false,1)then
				...
				if c_pic(aoc['lls']['登录按钮'],'登录按钮',true)then
					...
					up.lastname = bid_app
					up.phonename = sz.system.mgcopyanswer("UserAssignedDeviceName")
					up.password = '58速运-注册成功'
					up.phone = phone
					up.map = adid
					upFacebook(up)
				end
			else
				aoc['lls']['用车位置']={{126,427,0xffffff},{319,432,0xffffff},{513,432,0xffffff},}
				c_wz = math.random(1,3)
				click(aoc['lls']['用车位置'][c_wz][1],aoc['lls']['用车位置'][c_wz][2])
				if c_pic(aoc['lls']['登录按钮2'],'登录按钮2',true)then
					delay(2)
					up ={}
					up.mail = idfa
					up.fristname = app_name
		...
		elseif done(aoc['lls']['登录58速运2'],'登录58速运2',false,1)then
			if 取号 then
				if GET_Phone(id,token)then
					if c_pic(aoc['lls']['11位手机号2'],'11位手机号',true)then
						input(phone)
						click(443,307)
						delay(1)
						取号 = false
			...
			if c_pic(aoc['lls']['下一步2'],'下一步',true)then
				获取 = true
				delay(5)
			end
		...
		elseif done(aoc['lls']['选择城市'],'选择城市',true,3)then
...
app_url = 'https://itunes.apple.com/cn/app/id914882524?mt=8'
