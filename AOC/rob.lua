				...
				if k == '水' or k == '密银' then
					...
					if c_p(v,k,false)then
						if kuang_setting[1] and k == '金' then
							zy_mun = zy_mun + 2
						elseif kuang_setting[3] and k == '木' then
							zy_mun = zy_mun + 2
						else
							zy_mun = zy_mun + 1
						end
...
function auto_all()
	...
	while (os.time()-计时< 60* 15) do
		...
		elseif UI('在地图中','在地图界面',false,3,80)then
			delay(1)
			map_time = os.time()
			while ( 遗迹 and os.time()-map_time < 30) do
				keepScreen(true)
			...
			倒着找矿 = true
			if 遗迹 and UI_pic('在地图中','矿点顶部',false)then
				click(x,y+150)
				遗迹 = false
			elseif 遗迹 and UI_pic('在地图中','发现矿点',false)then
				click(x,y+150)
				遗迹 = false
			else
				--缩小地图()
...
function auto_fire()
	...
	while (os.time()-计时<超时) do
		if active(app,10)then
		elseif  遗迹数量 >= tonumber(values.kuang_mun) then
			return true
		elseif UI('在地图中','在地图界面',false,3,80)then
			delay(1)
			map_time = os.time()
			while (os.time()-map_time < 60) do
				keepScreen(true)
				if UI_pic('地图','正在跑路',false)or UI_pic('地图','正在跑路小',false)then
					delay(1)
				else
			...
			elseif 遗迹 and UI_pic('在地图中','发现矿点',false)then
				click(x,y+150)
				遗迹 = false
			elseif 遗迹 and UI_pic('在地图中','挡住矿的提示',false)then
				click(x+26,y+203)
				遗迹 = false
			else
				if 轮换 %2 == 0 then
		...
		elseif UI('返回','返回图标',false,1)then
			...
			elseif UI('返回','在世界地图上',false,1)then
				if setting[14] and 找图失败次数 <= 3 and 找金币()then
					delay(1)
					if map_one()then
...
function 战争学院()
	...
	while ((os.time()-计时<超时 )) do
		if active(app,5)then
		elseif UI('返回','战争学院界面')then
			if UI('返回','战争打完')then
				if UI('返回','战争打完',false,1)then
					if UI('返回','下一章',true,1)then
					else
						return true
					end
				end
